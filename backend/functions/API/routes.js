...
router.get("/hiringEvent/matches/:course", async (req, res) => {
  ...
  Matches.find({
    $or: [
      { course: req.params.course, status: "pending" },
      { course: req.params.course, status: "confirmed" },
    ],
  }).then((doc, err) => {
    console.log(doc);
...
router.get("/hiringEvent/rejectMatch/:name/:course", async (req, res) => {
  ...
  Matches.findOneAndUpdate(
    { course: req.params.course, name: req.params.name },
    { status: "rejected" }
  ).then((doc, err) => {
    console.log(doc);
    res.status(200).json(doc);
...
router.put("/create/hiringEvent", async (req, res) => {
  ...
  HiringEvent.create(newEvent).then((event) => {
    ...
    Notifications.create(newNotfication).then((event) => {
...
router.put("/notification/notify/user", async (req, res) => {
  ...
  Notifications.create(newNotification).then((event) =>
    res.status(200).json(event)
  );
});
...
router.get(
  "/notification/admin/receive/evaluation/:recipient",
  async (req, res) => {
    //return the message
    Notifications.find({ recipientEmail: req.params.recipient }).then((event) =>
      res.status(200).json(event)
    );
  }
);
  ...
  req.body.answers.forEach((element, i) => {
    if (element["Course Code"]) {
      ...
      newResponse = {
        ...
        applicantStatus:
          element["Applicant status ( 1- Fundable, 2-NotFundable,3-External)"],
...
router.put("/courses/update/", (req, res) => {
  ...
  Object.keys(courses).forEach((courseCode) => {
    ...
    Course.findOneAndUpdate(
      { hiringEventID: new ObjectId(hiringEventID), courseCode: courseCode },
      { applicantResponses: courses[courseCode] }
    ).then((element) => console.log());
  });
...
router.put("/update/course/hours", (req, res) => {
  ...
  ).then((element) => console.log("updated", req.body._id));
});
...
router.get("/get/tahour/:courseID", async (req, res) => {
  ...
  HiringEvent.find({ _id: "60401e61625a9ea848c092bc" }).then((event) =>
    res.status(200).json(event)
  );
...
router.get("/courses/getAll/:_id", (req, res) => {
  Course.find({ hiringEventID: new ObjectId(req.params._id) }).then((course) =>
    res.status(200).json(course)
  );
});

router.get("/courses/getAll/instructorID/:_id", (req, res) => {
  Course.find({ instructorID: new ObjectId(req.params._id) }).then((course) =>
    res.status(200).json(course)
  );
});
...
router.put("/update/matches", (req, res) => {
  ...
  Matches.findOneAndUpdate(
    ...
    {
      hoursFilled: currentMatch.hoursFilled,
      applicants: currentMatch.applicants,
    }
...
router.put("/notifications/update", (req, res) => {
  console.log(req.body.recipient, req.body.message);

  filter = { recipientEmail: req.body.recipient, message: req.body.message };
  update = { read: 1 };
